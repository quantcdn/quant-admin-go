...
[**ListMcpServers**](AIToolsAPI.md#ListMcpServers) | **Get** /api/v3/organizations/{organisation}/ai/mcp-servers | List MCP servers with gateway URLs
...
> ListMcpServers200Response ListMcpServers(ctx, organisation).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.AIToolsAPI.ListMcpServers(context.Background(), organisation).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AIToolsAPI.ListMcpServers``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ListMcpServers`: ListMcpServers200Response
	fmt.Fprintf(os.Stdout, "Response from `AIToolsAPI.ListMcpServers`: %v\n", resp)
}
```

...
type Compose struct {
	...
	// Optional. Forces single-task mode for data-safe applications (max one running task). When true: capacity is locked to 1. When false: explicitly allows scaling. When omitted: the platform auto-detects stateful containers and enables single-task mode if found.
	SingleTaskOnly NullableBool `json:"singleTaskOnly,omitempty"`
	// Optional. Seconds the load balancer waits after a task starts before an unhealthy health check can replace it (applied as the ECS service's healthCheckGracePeriodSeconds when a load balancer is attached). Raise for apps that are slow to boot, e.g. run migrations on startup. Tasks that become healthy sooner still enter service immediately. Defaults to 120 when omitted.
...
func NewCompose() *Compose {
	this := Compose{}
	var startupGracePeriodSeconds int32 = 120
...
func (o *Compose) SetSingleTaskOnlyNil() {
...
func (o *Compose) SetStartupGracePeriodSecondsNil() {
